...
function normalizeScreenId(id) {
  ...
  return screens[clean] ? clean : null;
}

function updateLocationHash(id, { replaceHash = false, syncHash = true } = {}) {
  if (!syncHash) return;
  const target = id === "hero" ? window.location.pathname + window.location.search : `#${id}`;
  const current = window.location.hash || window.location.pathname + window.location.search;
  if ((id !== "hero" && window.location.hash === target) || (id === "hero" && !window.location.hash)) return;
  const method = replaceHash ? "replaceState" : "pushState";
  window.history[method](null, "", target);
}

function showScreen(id, options = {}) {
  id = normalizeScreenId(id);
  if (!id) return;
  if (id === currentScreen) {
    updateLocationHash(id, options);
    return;
  }
  ...
  if (orbit.children.length !== modeData.length) {
    ...
    modeData.forEach((mode, index) => {
      ...
      button.addEventListener("click", () => {
        ...
        if (pos === 0) showScreen(mode.id);
        else rotateMode(pos === 3 ? -1 : 1);
      });
...
function rotateMode(step) {
  ...
  renderModeOrbit();
  orbitMotionTimer = window.setTimeout(() => orbit.classList.remove("is-rotating"), 560);
}
...
function handleOrbitTap(event) {
  ...
  if (x < rect.width * 0.36) rotateMode(-1);
  else if (x > rect.width * 0.64) rotateMode(1);
}

function handleOrbitTouchStart(event) {
  orbitTouchStartX = event.touches[0]?.clientX || 0;
}

function handleOrbitTouchEnd(event) {
  const endX = event.changedTouches[0]?.clientX || orbitTouchStartX;
  const delta = endX - orbitTouchStartX;
  if (Math.abs(delta) > 34) rotateMode(delta < 0 ? 1 : -1);
}

function openModal({ kicker, title, subtitle, drink, abv, meta, note, againText = "再来一次", onAgain }) {
  latestModalResult = { kicker, title, subtitle, drink, abv, meta, note };
...
function buildShareText() {
  if (!latestModalResult) return "我刚刚在酒精灵感异常中心生成了一份可疑快乐。";
  const { title, subtitle, drink, abv, meta, note } = latestModalResult;
  return [
    "我的酒精灵感异常档案",
    title,
    subtitle,
...
async function shareLatestResult() {
  ...
  const shareData = { title: "酒精灵感异常档案", text };
  try {
    if (navigator.share) {
      await navigator.share(shareData);
      announceStatus("结果已打开分享面板");
    } else if (navigator.clipboard?.writeText) {
      await navigator.clipboard.writeText(text);
      announceStatus("结果已复制，可以发给朋友了");
      flashShareButton("已复制");
    } else {
      window.prompt("复制这份结果发给朋友", text);
      announceStatus("请手动复制结果");
    }
  } catch (error) {
    if (error?.name !== "AbortError") announceStatus("分享没有成功，可以截图保存结果");
  }
}
...
function updateQuizProgress() {
  ...
  if (!form || !progress || !text) return;
  const count = new FormData(form).values();
  const done = [...count].length;
  const total = quizQuestions.length;
  progress.style.setProperty("--quiz-progress", `${Math.round((done / total) * 100)}%`);
...
document.getElementById("modeOrbit").addEventListener("click", (event) => {
  if (event.target.closest(".mode-can")) return;
  handleOrbitTap(event);
});
document.getElementById("modeOrbit").addEventListener("touchstart", handleOrbitTouchStart, { passive: true });
document.getElementById("modeOrbit").addEventListener("touchend", handleOrbitTouchEnd, { passive: true });
...
document.getElementById("quizForm").addEventListener("change", updateQuizProgress);
...
document.getElementById("shareResultBtn").addEventListener("click", shareLatestResult);
...
function syncScreenFromLocation() {
  const next = normalizeScreenId(window.location.hash);
  showScreen(next || "hero", { syncHash: false });
}
...
updateLab();
updateDock();
const initialScreen = normalizeScreenId(window.location.hash);
if (initialScreen && initialScreen !== currentScreen) {
  showScreen(initialScreen, { replaceHash: true });
}
document.getElementById("appLoading")?.classList.add("is-hidden");
attemptAutoMusic();
